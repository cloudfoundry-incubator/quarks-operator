	...
	Context("when using podstate TriggeredJob", func() {
		...
		Context("when using label matchers", func() {
			...
			It("should start a job for a matched pod", func() {
				...
				Expect(env.ContainJob(jobs, "job-exten-foo")).To(Equal(true))
				Expect(env.ContainJob(jobs, "job-slowj-foo")).To(Equal(true))
				Expect(env.ContainJob(jobs, "job-exten-bar")).To(Equal(true))
				Expect(env.ContainJob(jobs, "job-slowj-bar")).To(Equal(true))
