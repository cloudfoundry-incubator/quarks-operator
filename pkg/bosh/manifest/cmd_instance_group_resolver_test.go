	...
	Context("InstanceGroupResolver", func() {
		...
		Describe("BPMInfo", func() {
			...
			It("returns the bpm config for all jobs", func() {
				...
				Expect(bpm.Processes[0].Env["FOOBARWITHLINKINSTANCESADDRESS"]).To(Equal("doppler-z1-0"))
				Expect(bpm.Processes[0].Env["FOOBARWITHSPECADDRESS"]).To(Equal("log-api-z1-0"))
				...
				Expect(bpm.Processes[0].Env["FOOBARWITHSPECID"]).To(Equal("log-api-z1-0"))
