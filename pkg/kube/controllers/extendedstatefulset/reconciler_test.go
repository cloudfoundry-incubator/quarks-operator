	...
	Describe("Reconcile", func() {
		...
		Context("Provides a extendedStatefulSet definition", func() {
			...
			It("creates new statefulSet and continues to reconcile when new version is not available", func() {
				...
				err = client.Get(controllersconfig.NewBackgroundContext(), types.NamespacedName{Name: "foo", Namespace: "default"}, ess)
				Expect(err).ToNot(HaveOccurred())

				ss := &v1beta1.StatefulSet{}
				err = client.Get(controllersconfig.NewBackgroundContext(), types.NamespacedName{Name: "foo-v1", Namespace: "default"}, ss)
				Expect(err).ToNot(HaveOccurred())
			...
			It("updates existing statefulSet", func() {
				...
				ss := v1StatefulSet
				err = client.Create(controllersconfig.NewBackgroundContext(), ss)
				Expect(err).ToNot(HaveOccurred())
				...
				err = client.Create(controllersconfig.NewBackgroundContext(), pod)
				Expect(err).ToNot(HaveOccurred())
				...
				ss = &v1beta1.StatefulSet{}
				err = client.Get(controllersconfig.NewBackgroundContext(), types.NamespacedName{Name: "foo-v2", Namespace: "default"}, ss)
				Expect(err).ToNot(HaveOccurred())
		...
		Context("Provides a extendedStatefulSet containing ConfigMaps and Secrets references", func() {
			...
			It("creates new statefulSet and has correct resources", func() {
				...
				By("Adds OwnerReferences to all configurations", func() {
					err = client.Get(controllersconfig.NewBackgroundContext(), types.NamespacedName{Name: configMap1.Name, Namespace: "default"}, configMap1)
					Expect(err).ToNot(HaveOccurred())
					err = client.Get(controllersconfig.NewBackgroundContext(), types.NamespacedName{Name: configMap2.Name, Namespace: "default"}, configMap2)
					Expect(err).ToNot(HaveOccurred())
					err = client.Get(controllersconfig.NewBackgroundContext(), types.NamespacedName{Name: secret1.Name, Namespace: "default"}, secret1)
					Expect(err).ToNot(HaveOccurred())
					err = client.Get(controllersconfig.NewBackgroundContext(), types.NamespacedName{Name: secret2.Name, Namespace: "default"}, secret2)
					Expect(err).ToNot(HaveOccurred())
			...
			When("A ConfigMap reference is removed", func() {
				It("Creates new version and updates the config hash in the StatefulSet Annotations", func() {
					...
					err = client.Update(controllersconfig.NewBackgroundContext(), ess)
					Expect(err).ToNot(HaveOccurred())
					result, err = reconciler.Reconcile(request)
					Expect(err).ToNot(HaveOccurred())
					Expect(result).To(Equal(reconcile.Result{
			...
			When("A ConfigMap reference is updated", func() {
				It("Preserves current version and updates the config hash in the StatefulSet Annotations", func() {
					...
					err = client.Update(controllersconfig.NewBackgroundContext(), configMap1)
					Expect(err).ToNot(HaveOccurred())
					result, err = reconciler.Reconcile(request)
					Expect(err).ToNot(HaveOccurred())
					Expect(result).To(Equal(reconcile.Result{
		...
		Context("when there are three versions", func() {
			...
			It("cleans up old versions and stops reconcile when newer V3 is running", func() {
				...
				err := client.Update(controllersconfig.NewBackgroundContext(), v2Pod)
				Expect(err).ToNot(HaveOccurred())
				...
				err = client.Update(controllersconfig.NewBackgroundContext(), v3Pod)
				Expect(err).ToNot(HaveOccurred())
				...
				err = client.Get(controllersconfig.NewBackgroundContext(), types.NamespacedName{Name: "foo-v3", Namespace: "default"}, ss)
				Expect(err).ToNot(HaveOccurred())
