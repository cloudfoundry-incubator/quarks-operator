...
var _ = Describe("Store", func() {
	...
	hasSecret := func(name string) bool {
		...
		err := client.Get(context.TODO(), crc.ObjectKey{Namespace: namespace, Name: name}, secret)
		return err == nil
	}
	...
	secretCount := func() int {
		...
		err := client.List(context.TODO(), crc.InNamespace(namespace), secrets)
		Expect(err).ToNot(HaveOccurred())
	...
	Describe("Save", func() {
		Context("when there is no versioned manifest", func() {
			It("should create the first version", func() {
				err := store.Save(exampleManifest(`{"instance-groups":[{"instances":3,"name":"diego"},{"instances":2,"name":"mysql"}]}`), exampleSourceDescription)
				Expect(err).ToNot(HaveOccurred())
		...
		Context("when there already is a version of the manifest", func() {
			...
			It("should create a new version", func() {
				err := store.Save(exampleManifest(`{"instance-groups":[{"instances":3,"name":"diego"}]}`), exampleSourceDescription)
				Expect(err).ToNot(HaveOccurred())
				name := fmt.Sprintf("deployment-%s-%d", deploymentName, 1)
				Expect(hasSecret(name)).To(BeTrue())
				...
				err = store.Save(exampleManifest(`instance-groups: []`), exampleSourceDescription)
				Expect(err).ToNot(HaveOccurred())
	...
	Describe("Delete", func() {
		Context("when a manifest with multiple version exists", func() {
			...
			It("should get rid of all versions of a manifest", func() {
				currentManifestSecrets, err := store.List()
				Expect(err).ToNot(HaveOccurred())
		...
		Context("when there is a manifest with multiple versions", func() {
			BeforeEach(func() {
				for i := 1; i < 10; i++ {
					...
					err = NewStore(client, namespace, "another-deployment").Save(exampleManifest(`instance-groups: []`), exampleSourceDescription)
					Expect(err).ToNot(HaveOccurred())
				}
			})
		...
		Context("when there is a manifest with multiple versions", func() {
			BeforeEach(func() {
				for i := 1; i < 10; i++ {
					...
					err := store.Save(m, exampleSourceDescription)
					Expect(err).ToNot(HaveOccurred())
	...
	Describe("RetrieveVersionSecret", func() {
		...
		It("should retrieve the secret for specified version", func() {
			secret, err := store.RetrieveVersionSecret(1)
			Expect(err).ToNot(HaveOccurred())
